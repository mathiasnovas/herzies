...
//#endregion
//#region ../shared/dist/leveling.js
...
function xpToNextLevel(herzie) {
	return xpForLevel(herzie.level + 1) - (herzie.xp - totalXpForLevel(herzie.level));
}
function levelProgress(herzie) {
...
//#endregion
//#region src/renderer/HerzieArt.tsx
...
function injectFace(lines, eye, mouth) {
	return lines.map((line) => line.replace("EYE", eye).replace("MTH", mouth));
}
function composeHerzie(appearance, stage) {
	if (stage === 3) {
	...
	return {
		lines: [pick(S1_ACCESSORIES, appearance.accessoryIndex), ...injectFace(pick(S1_HEADS, appearance.headIndex), pick(S1_EYES, appearance.eyesIndex), pick(S1_MOUTHS, appearance.mouthIndex))],
		accentLineCount: 1,
		limbAccent: false
	};
}
function HerzieArt({ appearance, stage = 1, size = 16, animate = true }) {
	const [tick, setTick] = (0, import_react.useState)(0);
	const [herzie, setHerzie] = (0, import_react.useState)(appearance ?? null);
	...
	let finalLines = lines.map((line, i) => {
		if (i < accentLineCount) return i === 0 ? {
			text: line,
			color: accent
		} : {
			text: line,
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("pre", {
		...
		children: finalLines.map((l, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			style: {
				color: l.color,
				display: "block"
			},
...
//#endregion
//#region src/renderer/ItemDisplay.tsx
function ItemDisplay({ item, size = 10, animate = true }) {
	const [frame, setFrame] = (0, import_react.useState)(0);
	(0, import_react.useEffect)(() => {
		if (!animate || item.frames.length <= 1) return;
		const id = setInterval(() => setFrame((f) => (f + 1) % item.frames.length), 80);
		return () => clearInterval(id);
...
function frontTexture(u, v) {
	const bw = .055;
	if (u < bw || u > 1 - bw || v < bw || v > 1 - bw) return .9;
	const ib = .11, ibw = .012;
	if (u > ib - ibw && u < ib + ibw && v > ib && v < 1 - ib || u > 1 - ib - ibw && u < 1 - ib + ibw && v > ib && v < 1 - ib || v > ib - ibw && v < ib + ibw && u > ib && u < 1 - ib || v > 1 - ib - ibw && v < 1 - ib + ibw && u > ib && u < 1 - ib) return .65;
	for (const [dx, dy] of [
		[.16, .1],
		[.84, .1],
		[.16, .9],
		[.84, .9]
	]) if (Math.abs(u - dx) / .035 + Math.abs(v - dy) / .045 < 1) return 1;
	const cx = .5, cy = .5;
	const blocks = [
		[
			cx - .03,
	...
	for (const [x1, y1, x2, y2] of blocks) if (u >= x1 && u <= x2 && v >= y1 && v <= y2) return .95;
	return .3;
}
function backTexture(u, v) {
	const bw = .055;
	if (u < bw || u > 1 - bw || v < bw || v > 1 - bw) return .7;
	const g = .065, w = .018;
	if (u % g < w || v % g < w) return .5;
	return .28;
}
function project(p) {
	const s = CAM / (p[2] + CAM);
	return [SW / 2 + p[0] * s * SW * .22 * CHAR_ASPECT, SH / 2 + p[1] * s * SH * .28];
}
function triUV(px, py, ax, ay, au, av, bx, by, bu, bv, cx, cy, cu, cv) {
	...
	const inv = 1 / den;
	const s = (d11 * d02 - d01 * d12) * inv, t = (d00 * d12 - d01 * d02) * inv;
	if (s < -.001 || t < -.001 || s + t > 1.001) return null;
	const w = 1 - s - t;
	return [w * au + t * bu + s * cu, w * av + t * bv + s * cv];
}
function renderCardFrame(yAngle) {
	const xf = CORNERS.map((v) => rotY(rotZ(v, TILT), yAngle));
	const faceN = normV(cross([
		xf[1][0] - xf[0][0],
		xf[1][1] - xf[0][1],
		xf[1][2] - xf[0][2]
	], [
	...
	const bright = Array.from({ length: SH }, () => Array(SW).fill(-1));
	const isContent = Array.from({ length: SH }, () => Array(SW).fill(false));
	for (let sy = 0; sy < SH; sy++) for (let sx = 0; sx < SW; sx++) {
		const px = sx + .5, py = sy + .5;
		const uv = triUV(px, py, pr[0][0], pr[0][1], UVS[0][0], UVS[0][1], pr[1][0], pr[1][1], UVS[1][0], UVS[1][1], pr[2][0], pr[2][1], UVS[2][0], UVS[2][1]) ?? triUV(px, py, pr[0][0], pr[0][1], UVS[0][0], UVS[0][1], pr[2][0], pr[2][1], UVS[2][0], UVS[2][1], pr[3][0], pr[3][1], UVS[3][0], UVS[3][1]);
		if (!uv) continue;
		let [u, v] = uv;
		if (!front) u = 1 - u;
		const tex = front ? frontTexture(u, v) : backTexture(u, v);
		const lit = tex * (.2 + .8 * diffuse);
		bright[sy][sx] = lit;
		isContent[sy][sx] = front && tex > .5;
	}
	return bright.map((row, y) => row.map((val, x) => {
		if (val < 0) return " ";
		const ch = RAMP[Math.min(Math.floor(val * 11), 11)];
		if (ch === " ") return " ";
		return isContent[y][x] ? col("#FFD700", ch) : front ? col("#8B6914", ch) : col("#654A0E", ch);
	}).join(""));
}
var CD_RADIUS = 1.1;
var CD_CORNERS = [
...
function cdFrontTexture(u, v) {
	...
	if (r < .1) return .9;
	const ring = r * 40 % 1;
	const base = .3 + .4 * (1 - r / .5);
	if (ring < .15) return base + .2;
	if (r > .14 && r < .25) return .55;
	return base;
}
function cdBackTexture(u, v) {
	const dx = u - .5, dy = v - .5;
	const r = Math.sqrt(dx * dx + dy * dy);
	if (r > .5) return -1;
	if (r < .08) return -1;
	if (r < .1) return .7;
	return .35 + .15 * Math.sin(r * 30);
}
function renderCdFrame(yAngle) {
	...
	return bright.map((row) => row.map((val) => {
		if (val < 0) return " ";
		const ch = RAMP[Math.min(Math.floor(val * 11), 11)];
		if (ch === " ") return " ";
		return val > .6 ? col("#E8E8E8", ch) : val > .35 ? col("#C0C0C0", ch) : col("#808080", ch);
	}).join(""));
}
function generateFrames(renderFn, count = 36) {
	return Array.from({ length: count }, (_, i) => renderFn(i / count * Math.PI * 2));
}
var firstEditionFrames = generateFrames(renderCardFrame);
var cdFrames = generateFrames(renderCdFrame);
var ITEMS = [{
	id: "first-edition",
...
function HomeView({ state }) {
	const { herzie, nowPlaying, multipliers, isOnline } = state;
	if (!herzie) return null;
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					...
					children: herzie.name
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					style: {
						fontSize: 10,
						color: isOnline ? "#4ade80" : "#f87171",
						background: isOnline ? "#4ade8020" : "#f8717120",
						padding: "2px 8px",
						borderRadius: 8
					},
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HerzieArt, {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["Level ", herzie.level] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["Stage ", herzie.stage] })]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						style: {
							background: "#333",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [Math.ceil(toNext), " XP to next"]
					})
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						style: { color: "#facc15" },
						children: herzie.currency
					}), " H"] }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						style: { color: "#7dd3fc" },
						children: herzie.friendCodes.length
					}), " friends"] }),
					herzie.streakDays > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						style: { color: "#facc15" },
						children: [herzie.streakDays, "d"]
					}), " streak"] })
				]
			}),
			!multipliers ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				style: {
					fontSize: 10,
					color: "#555",
					marginBottom: 6
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					style: { color: "#facc15" },
					children: "Bonuses:"
				}), " Log in to get bonuses"]
			}) : activeMultipliers.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				style: { marginBottom: 6 },
				children: activeMultipliers.map((m) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					style: {
						fontSize: 10,
						display: "flex",
						justifyContent: "space-between"
			...
			nowPlaying ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				style: {
					borderTop: "1px solid #333",
					paddingTop: 6
				},
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					children: "Play some music to start earning XP"
				})
			})
		]
	});
}
function FriendsView({ herzie, onStartTrade }) {
	const [friends, setFriends] = (0, import_react.useState)({});
	const [addCode, setAddCode] = (0, import_react.useState)("");
	const [message, setMessage] = (0, import_react.useState)("");
	const loadFriends = (0, import_react.useCallback)(async () => {
		if (herzie.friendCodes.length === 0) return;
		setFriends(await window.herzies.friendLookup(herzie.friendCodes));
	}, [herzie.friendCodes]);
	(0, import_react.useEffect)(() => {
		loadFriends();
	}, [loadFriends]);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: ["Your code: ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					style: {
						color: "#7dd3fc",
						fontWeight: "bold"
					},
					children: herzie.friendCode
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				style: {
				...
				children: herzie.friendCodes.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					style: {
						fontSize: 12,
						color: "#555",
						textAlign: "center",
					...
					children: "No friends yet. Share your code above!"
					...
					return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							...
							children: profile?.name ?? code
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							style: {
								fontSize: 10,
								color: "#666"
							},
							children: profile ? `Lv.${profile.level} · Stage ${profile.stage}` : code
						})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
								gap: 4
...
function InventoryView({ herzie }) {
	const [inventory, setInventory] = (0, import_react.useState)(null);
	const [currency, setCurrency] = (0, import_react.useState)(herzie.currency);
	...
	const handleSell = async (itemId, qty) => {
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ItemDisplay, {
						item: selected,
						size: 9
					})
				}),
				...
				selected.sellPrice && qty > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					style: {
						fontSize: 11,
						color: "#666",
						marginBottom: 4
					...
					children: [
						...
						qty >= 5 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							style: btnStyle,
							onClick: () => handleSell(selectedItem, 5),
							children: "Sell 5"
						}),
						qty > 1 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
							style: btnStyle,
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: "No items yet. Keep listening to earn drops!"
		}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			style: {
				flex: 1,
				overflow: "auto"
			},
			children: items.map(([itemId, qty]) => {
...
function TradeView({ herzie, initialTarget, onClose }) {
	const [targetCode, setTargetCode] = (0, import_react.useState)(initialTarget ?? "");
	const [tradeId, setTradeId] = (0, import_react.useState)(null);
	...
	(0, import_react.useEffect)(() => {
		if (initialTarget && !tradeId) handleCreate(initialTarget);
	}, []);
	const handleCreate = async (overrideCode) => {
	...
	const handleLock = async () => {
		if (tradeId) await window.herzies.tradeLock(tradeId);
	};
	const handleAccept = async () => {
		if (tradeId) await window.herzies.tradeAccept(tradeId);
	};
	if (!tradeId) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		style: {
			display: "flex",
			flexDirection: "column",
			height: "100%"
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: "Enter a friend's code to start a trade"
			})
		]
	});
	const myOffer = trade ? trade.initiatorName === herzie.name ? trade.initiatorOffer : trade.targetOffer : null;
	const theirOffer = trade ? trade.initiatorName === herzie.name ? trade.targetOffer : trade.initiatorOffer : null;
	const myLocked = trade ? trade.initiatorName === herzie.name ? trade.state === "initiator_locked" || trade.state === "both_locked" : trade.state === "target_locked" || trade.state === "both_locked" : false;
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: ["Trading with ", trade ? trade.initiatorName === herzie.name ? trade.targetName : trade.initiatorName : "..."]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					style: {
						...btnStyle,
						fontSize: 10,
			...
			!trade || trade.state === "pending" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				style: {
					fontSize: 12,
					color: "#555",
					textAlign: "center",
				...
				children: "Waiting for them to join..."
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: ["Your offer ", myLocked ? "🔒" : ""]
					}),
					myOffer && Object.entries(myOffer.items).map(([id, qty]) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						style: {
							fontSize: 11,
							color: "#ccc"
						},
					...
					myOffer && myOffer.currency > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						style: {
							fontSize: 11,
							color: "#facc15"
						},
						children: [myOffer.currency, " H"]
					}),
					myOffer && Object.keys(myOffer.items).length === 0 && myOffer.currency === 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						style: {
							fontSize: 10,
							color: "#ccc"
						},
					...
					theirOffer && Object.entries(theirOffer.items).map(([id, qty]) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						style: {
							fontSize: 11,
							color: "#ccc"
						},
					...
					theirOffer && theirOffer.currency > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						style: {
							fontSize: 11,
							color: "#facc15"
						},
						children: [theirOffer.currency, " H"]
					}),
					theirOffer && Object.keys(theirOffer.items).length === 0 && theirOffer.currency === 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						style: {
							fontSize: 10,
							color: "#ccc"
						},
				...
				children: [!myLocked && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					style: btnStyle,
					onClick: handleLock,
					children: "Lock offer"
				}), bothLocked && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					style: {
						...btnStyle,
						color: "#4ade80"
					},
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					children: "Account"
				}), state.isOnline ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					style: {
						...btnStyle,
						color: "#f87171"
					},
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: ["Herzies Desktop v", state.version]
			})
		]
	});
}
...
function SplashScreen() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			...
			children: loggingIn ? "Opening browser..." : "Login"
		})]
	});
}
function App() {
	...
	const [tradeTarget, setTradeTarget] = (0, import_react.useState)(null);
	(0, import_react.useEffect)(() => {
		window.herzies.getState().then(setState);
		return window.herzies.onStateUpdate(setState);
	}, []);
	const { herzie } = state;
	if (!state.isOnline) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SplashScreen, {});
	const handleStartTrade = (code) => {
		setTradeTarget(code);
		setView("trade");
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				view === "home" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HomeView, { state }),
				view === "friends" && herzie && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FriendsView, {
					herzie,
					onStartTrade: handleStartTrade
				}),
				view === "inventory" && herzie && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(InventoryView, { herzie }),
				view === "trade" && herzie && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TradeView, {
					herzie,
				...
				view === "settings" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SettingsView, { state })
			]
		}), herzie && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabBar, {
			view,
			setView
		})]
	});
