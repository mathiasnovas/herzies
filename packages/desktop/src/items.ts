...
const CORNERS: V3[] = [[-CARD_HW, -CARD_HH, 0], [CARD_HW, -CARD_HH, 0], [CARD_HW, CARD_HH, 0], [-CARD_HW, CARD_HH, 0]];
const UVS: V2[] = [[0, 0], [1, 0], [1, 1], [0, 1]];
...
function frontTexture(u: number, v: number): number {
  ...
  const ib = 0.11, ibw = 0.012;
  if ((u > ib - ibw && u < ib + ibw && v > ib && v < 1 - ib) || (u > 1 - ib - ibw && u < 1 - ib + ibw && v > ib && v < 1 - ib) || (v > ib - ibw && v < ib + ibw && u > ib && u < 1 - ib) || (v > 1 - ib - ibw && v < 1 - ib + ibw && u > ib && u < 1 - ib)) return 0.65;
  const diamonds: V2[] = [[0.16, 0.1], [0.84, 0.1], [0.16, 0.9], [0.84, 0.9]];
  for (const [dx, dy] of diamonds) { const du = Math.abs(u - dx) / 0.035; const dv = Math.abs(v - dy) / 0.045; if (du + dv < 1) return 1.0; }
  const cx = 0.5, cy = 0.5;
  const blocks: [number, number, number, number][] = [[cx - 0.03, cy - 0.16, cx + 0.03, cy + 0.13], [cx - 0.08, cy - 0.13, cx - 0.02, cy - 0.09], [cx - 0.055, cy - 0.16, cx - 0.02, cy - 0.13], [cx - 0.09, cy + 0.13, cx + 0.09, cy + 0.17]];
  for (const [x1, y1, x2, y2] of blocks) { if (u >= x1 && u <= x2 && v >= y1 && v <= y2) return 0.95; }
  return 0.3;
}
...
function triUV(px: number, py: number, ax: number, ay: number, au: number, av: number, bx: number, by: number, bu: number, bv: number, cx: number, cy: number, cu: number, cv: number): V2 | null {
...
function renderCardFrame(yAngle: number): string[] {
  ...
  const isContent: boolean[][] = Array.from({ length: SH }, () => Array(SW).fill(false));

  for (let sy = 0; sy < SH; sy++) {
    for (let sx = 0; sx < SW; sx++) {
      const px = sx + 0.5, py = sy + 0.5;
      const uv = triUV(px, py, pr[0][0], pr[0][1], UVS[0][0], UVS[0][1], pr[1][0], pr[1][1], UVS[1][0], UVS[1][1], pr[2][0], pr[2][1], UVS[2][0], UVS[2][1]) ??
        triUV(px, py, pr[0][0], pr[0][1], UVS[0][0], UVS[0][1], pr[2][0], pr[2][1], UVS[2][0], UVS[2][1], pr[3][0], pr[3][1], UVS[3][0], UVS[3][1]);
      if (!uv) continue;
      let [u, v] = uv;
      if (!front) u = 1 - u;
...
// --- CD rendering ---
const CD_RADIUS = 1.1;
const CD_CORNERS: V3[] = [[-CD_RADIUS, -CD_RADIUS, 0], [CD_RADIUS, -CD_RADIUS, 0], [CD_RADIUS, CD_RADIUS, 0], [-CD_RADIUS, CD_RADIUS, 0]];
...
function renderCdFrame(yAngle: number): string[] {
  ...
  return bright.map((row) => row.map((val) => {
    ...
    const ch = RAMP[idx];
    if (ch === " ") return " ";
    return val > 0.6 ? col("#E8E8E8", ch) : val > 0.35 ? col("#C0C0C0", ch) : col("#808080", ch);
  }).join(""));
}

// --- Generate all frames ---
function generateFrames(renderFn: (angle: number) => string[], count = 36): string[][] {
