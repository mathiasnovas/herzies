...
export function classifyGenre(spotifyGenres: string[]): Genre[] {
	...
	for (const raw of spotifyGenres) {
		...
		if (lower.includes("rap") || lower.includes("trap") || lower.includes("drill")) {
			matched.add("hip-hop");
		}
		if (lower.includes("edm") || lower.includes("house") || lower.includes("techno") || lower.includes("dubstep")) {
			matched.add("electronic");
		}
		if (lower.includes("alt") || lower.includes("shoegaze") || lower.includes("dream pop")) {
			matched.add("indie");
		}
		if (lower.includes("hardcore") || lower.includes("death") || lower.includes("thrash")) {
			matched.add("metal");
		}
		if (lower.includes("reggaeton") || lower.includes("salsa") || lower.includes("bachata")) {
			matched.add("latin");
		}
